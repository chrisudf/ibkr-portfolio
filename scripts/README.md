...
- **`Statement generation in progress` 卡很久**: 每次拉取的轮询预算是
  `FLEX_MAX_POLLS × FLEX_POLL_INTERVAL`，代码默认 120 × 15s = 30 分钟，本
  部署 2026-09-16 起是 240 × 15s = **3600 秒**，也就是 `BUDGET_CEILING_SEC`
  的上限。刷新按钮不会因此卡住 —— 它拿到 202 就返回，进度走
  `/api/refresh/status`。

  **先看周几**：只有「需要并入一个新交易日」的日子才慢。内容没变化的日子
  （周一、假期后）是秒回 —— 09-07 是 18 秒，09-14 是 57 秒，抓回来的字节数
  和上一次逐字节相同。

  慢的那些日子，生成耗时在**持续增长**：09-08 1340s、09-10 1837s、
  09-12 1794s、09-15 2337s，约 +142 秒/天。同一周报表只大了 1.9%
  （StatementOfFunds 957 → 975 行），所以**耗时和数据量已经脱钩**。

  预算再不够时，下一步不是继续加数字（也加不动了，3600 就是上限），而是把
  报表本身改小：把 `StatementOfFunds`（2005 行，最大的一块）拆成单独的低频
  query。`NAV` 每账户 262 行**不能动** —— 净值曲线和年度收益率全靠它。
  详见 `lesson.md` 第 19 条。
