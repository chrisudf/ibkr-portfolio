...
**两个方向指标故意不合并。**「净人头」按管理人数计（建仓/清仓算两票，加仓/减仓算一票），
「净股数」按股数计。两者相反的时候恰恰信息量最大 —— Q2 2026 的 Alphabet：27 家在减
（人头 −27），但 Berkshire 一家两类合计加了 4,800 万股（股数 +3,090 万）。合成一个
「情绪分」会把这一行报反。表格按**股数**排序。
