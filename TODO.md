...
## ~~[P2] 净值曲线 —— 注资期的现金流与 NAV 行对不上~~ ✓ 已完成（2026-08-28）

`fundedSeries()` 先裁掉了注资前的残根（$0 / $0.65 那几天），把 U228 从
`TWR −99.1% / maxDD −99.5%` 拉回了合理区间；剩下的注资爬坡期已由**现金流
改按 SoF `ReportDate`（入账日）记账**根治 —— 见
`parser/ibkr_flex_csv.py::_ingest_statement_of_funds` 与
`tests/test_parsers.py` 里 ReportDate 那一组用例。

真 CSV（multi2，2025-08-27 → 2026-08-26）逐笔重算的实测效果：

| | U228 修复前 | U228 修复后 | U174（对照） |
|---|---|---|---|
| 年化 TWR | 117.2% | **91.4%** | 35.9% → 34.8% |
| 年化波动 | 88.9% | **43.5%** | 46.1% → 44.7% |
| 最大回撤 | −24.4%（11-14→11-20） | **−21.7%（2026-01-28→03-30）** | −29.3%，窗口不变 |
| 最好链环 | **+64.0%** | **+11.5%** | — |
| 最差链环 | −23.7% | −9.1% | — |

两个判据都对上了：波动 88.9% → 43.5%，正好落在下面「起点敏感性」表里
12-02 之后那段稳定值（45.2% / 46.0%）——说明 11 月那几个主导方差的链节
真的没了，整个窗口现在可用；最大回撤也从注资周挪到了 2026 年那段真实回撤。
U174 作为对照几乎没动，回撤窗口一天没变。

**下面的调查记录整段保留**，因为结论（成因、机制、为什么不能按 NAV 差值
猜顺延）就是这个修法的依据。
...
- ✓ **日期错配（成因 1 与 2 是同一个病，一并治）** —— 已落地。**不按
  `NAV_i − NAV_{i−1}` 差值猜要不要顺延**：实测滞后是 −1 ~ +14 天、一轮
  注资还拆成多笔错峰到账，没有任何固定偏移能拟合，按差值猜一天根本不够。
  改成直接用 SoF 每行现成的 `ReportDate`（入账日，与 NAV 区块同一套日期
  框架）记账，`Date`（电汇发起日）只在没有 `ReportDate` 的老版式里兜底。
  真 CSV 20 条外部流水的滞后分布：`0` 天 11 条、`−1` 天 6 条、`+1/+10/+14`
  各 1 条。
- ✓ **不需要去重**：每笔流水单条记录、无双日期双计（真 CSV 已验证）。
  `txn:` id 不受影响；`synth:` 兜底键跟着换到入账日，同日同额靠出现序号
  区分，有用例钉住。
- ✓ **`<=` 边界一并消失**：`navStats` 的「日期 ≤ 首点 ⇒ 已含在首点 NAV 里」
  在按入账日分桶后才真正成立 —— 同一入账日的钱本来就在那天的 NAV 里。
- ✓ **「TWR 链永远不改写」的原则保持**：这次是在**源头**把日期对齐，没有
  给链条加任何改写/平滑；`fundedSeries` 也只裁前导段、不动链环。
- **仍在菜单上的残留**（很窄）：一笔**从未在任何 NAV 行印出来**的入金
  （当天穿仓、或隔日原路退回的过路电汇）。它没有入账日可对齐，只能靠
  流水本身发现。没实例，先不处理。
  ~~中途崩盘后再注资~~ 也随重对齐一起好了：钱记在 NAV 反映它的那天，
  分母里有它、当天 NAV 里也有它，链环回到正常量级。

### 影响面

只影响**账户注资爬坡期**在窗口内的情况。U174 全程都是注资完成后的状态，
262 个点一个没被裁，数字本来就大致是对的 —— 修复前后 TWR 35.9% → 34.8%、
maxDD 窗口一天没变，正好当回归基准。
