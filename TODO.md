...
**1. 入金日期比 NAV 行早一天。** `navStats` 用 begin-of-day 约定
（`denom = 前一日 NAV + 区间内入金`），但 IBKR 把现金流记在 D 日、当天的
NAV 行却还没反映它，D+1 才涨上去：
...
**2. 有一笔约 $39.5k 的流入根本不在 `cash_flows` 里。**

```
11-26 → 11-27   NAV $48,660 → $101,245  (+$52,585)   计入入金只有 $13,068   r = +64.0%
```

一天里 NAV 涨 $52.6k，记录在案的入金只有 $13.1k，剩下 $39.5k 在一个 $48.6k
的账户上是 +81% 的「单日行情」—— 不可能。最像**转入的持仓（ACATS 之类）**，
它不是现金交易，所以 Cash Transactions / Statement of Funds 都抓不到。

### 修法（待办）

- **日期错配**：不要按现金流自己的日期分桶，改成落到「NAV 序列里第一个真正
  反映了它的观测点」。可以拿 `NAV_i − NAV_{i−1}` 与该笔流水比对来判定要不要
  顺延一天。注意别把真实的单日大涨误判成错配。
- **非现金流入**：先确认 Flex Query 里有没有能拿到 position transfer 的 section
  （Transfers / ChangeInPositionValue 之类）。抓不到的话，退而求其次：当某个
  链节的 NAV 变化与已知流水差得离谱时，在面板上**标注这段窗口不可信**，
  而不是照常打印一个 TWR。
